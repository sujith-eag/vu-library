...
DESCRIPTION
       ...
       -n, --no-clobber
              do not overwrite an existing file and do not fail (overrides a -u or pre‐
              vious -i option). See also --update; equivalent to --update=none.
       ...
       ATTR_LIST is a comma-separated list of attributes.  Attributes  are  'mode'  for
       permissions  (including any ACL and xattr permissions), 'ownership' for user and
       group, 'timestamps' for file timestamps, 'links' for hard links,  'context'  for
       security context, 'xattr' for extended attributes, and 'all' for all attributes.
       ...
       or if --reflink=auto is specified, fall back to  a  standard  copy.   Use  --re‐
